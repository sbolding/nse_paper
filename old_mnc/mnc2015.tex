...
method linearizes Eq.~\eqref{ho_cont} \& Eq.~\eqref{lo_cont} over a discrete time
...
the time derivative treated continously. The MC solution for energy depositon by the
radiaiton is used to directly estimate
a new end of time step temperature in each cell of a fixed mesh.  In optically thick regions, or for
...
regions based on a discrete diffusion approximation~\cite{imd,ddmc}. In IMC the
...
In the HOLO context, sufficient MC histories must be performed to eliminate statistical
noise in the consistency terms that can contaminate the LO solution.
Exponentially-convergent Monte Carlo provides an algorithm that can efficiently
...
exceptionally good in the time-dependent TRT problems because information about the
intensity from the previous time step can be used as an accurate initial guess to the
problem. Additionally, no particle histories are required in regions of the problem
where the problem is in equilibrium, similar to~\cite{rmc}.  However, implementation
of ECMC is non-trivial, requiring a functional form of the solution in all
phase-space variables that must be sampled from, and Monte Carlo sources that produce
particels of both positive and negative weights.


Our algorithm contains similarities to~\cite{rmc}. Essentially, a single batch of
ECMC is the first batch of the residual MC~\cite{rmc} HO solver in.  OUr method gives us
an estimate of the error in each of the moments (with some unknown statistical
uncertainty).  We also have a LDFE trial space for the temperature and radiation
spatial representaiton, allowing for the discretization to preserve the equilibrium
diffusion limit.  The formulation of the residual in~\cite{rmc} can produce minimal
statistical noise in slowly varying problems where the behavior of the system is near
equilbrium. In problems where there is greater variance over a time step.  The LO
system in~\cite{rmc} assumes an ansantz relation between the face and volume averaged
values, which instead we will ultimately enforce strict spatial consistency with an
LD FE representation and angularly we have simply taken integrals of the transport
equation.  Generally, the minimum number of histories to obtain convergence with the
LDFE trial space is larger than a piece-wise constant to accurately estimate the
first moment in $x$ and $\mu$.
...
exact solution. In general, for TRT problems, optically thick and slowly varying
...
\begin{equation}
    I_{x,ij} = \frac{6}{\int\limits_{\mu_{j-1/2}}^{\mu_{j+1/2}}
...
path in the cell as
\begin{equation}
    I_{x,ij} \simeq \frac{1}{h_xh_\mu} \int_{\Delta s_j}  w(x,\mu)
\end{equation}
where $w(x,\mu)$ is the normalized weight in the MC simulation. 
