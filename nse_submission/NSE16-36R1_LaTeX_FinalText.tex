...
\colorlet{shadecolor}{blue!16}
%\newcommand{\sh}[1]{{\color{shadecolor} #1 }}   %used to highlight changes
\newcommand{\sh}[1]{{#1}}   %highlights are removed
...
\sh{absorption and reemission physics lead to systems that require solution in a mix of
...
method partially linearizes Eq.~\eqref{ho_cont} \& Eq.~\eqref{t_cont} over a discrete time
...
to resolve, e.g., absorption-reemission and physical scattering events, are moved to the LO
...
\sh{The formulation of the residual in~\cite{rmc} use an estimate of the solution such
...
In the HOLO context, the LO solver models \sh{isotropic scattering} and
...
\sh{Eq.~\eqref{ho_trans} and Eq.~\eqref{lo_mat}}, formed over a spatial finite element
...
Eddington factor. \sh{ If the angular consistency parameters were exact, then the LO equations
...
\sh{The LO system uses a LDFE spatial discretization for the temperature and half-range scalar intensities.  The LDFE spatial discretization should correctly preserve the equilibrium diffusion limit, a critical aspect
for TRT equations~\cite{larsen_edl,morel_newton}.  Additionally, the implicit time discretization with sufficient convergence of the nonlinear emission source will 
ensure that the method will not exhibit maximum principle violations~\cite{larsen_mpv}.}
...
Eq.~\eqref{ho_trans}.   The LDFE representation of the emission source mitigates teleportation error.
...
sources; \sh{the representation of sources is similar to the linear moments method
...
typical operator splitting stability issues that require linearization of the emission
source.
...
intensity-weighted
averages of $\mu$, which \sh{we denote as consistency terms}.  As an example, the equation resulting from application of the $L$ moment and
...
interpolant $T^4(x)\simeq T_{L,i}^4 b_{L,i}(x) + T_{R,i}^4 b_{R,i}(x)$.   \sh{The $L$ and $R$ spatial moments are taken of the material
...
as an approximation for $I^{n+1}(x,\mu)$. \sh{To close the LO system spatially, a linear-discontinuous (LD) spatial closure with the usual upwinding
...
Because there are \sh{no derivatives of $T$} in Eq.~\eqref{lo_mat}, there is no need
...
in cells where \sh{negative intensities} occur.  For simplicity, we also use standard FE lumping for
$T(x)$ and $T^4(x)$ in the material balance equations, e.g., Eq.~\eqref{lo_mat_dis}. 
...
ECMC method can be found in~\cite{jake}, but a brief overview is given here.  \sh{A
    general proof of exponential convergence for related adaptive MC transport methods
    with a different formulation is depicted in~\cite{spanier_mc}.}

    \sh{In operator notation, Eq.~\eqref{eq:ho_base} can be written as}
...
The linear operator $\B L^k$ is the \sh{continuous} streaming plus
...
\end{equation}
where $I^{n+1}$ is the exact solution and ${\epsilon}^{(m)}$ is the \sh{true error} in
$\tilde{I}^{n+1,(m)}$. 
...
computed using an equivalent tally, preserving the zeroth moment of the error.
...
$I^{n+1,(m+1)}$ and $\epsilon^{(m)}$ are correlated through $I^{n+1,(m)}$ and the MC
source $r^{(m)}$.  }
%Although th
...
\sh{For the HO solver, in cells near the radiation wavefront, the LDFE trial space results in
    negative values of $\tilde{I}^{n+1}(x,\mu)$, similar to the LO solver.}  Because the residual formulation in ECMC allows for negative weight
...
continuously deposit weight until they reach 0.25\% of their original weight. \sh{ Path lengths are tracked in terms of mfp, so there is no need to resample at material
interfaces.
...
\sh{a source tilting algorithm for two test problems~\cite{jayenne}.  Also, we
...
\sh{Although isotropic scattering is handled by the LO solver in the algorithm described above, we have only
...
similar agreement in the location of the \sh{wavefront} due to the linear shape of the emission source over a cell.  The cells
...
for the same numbers of \sh{histories, producing FOM values that are two orders of
magnitude greater than for IMC.}  Where as the FOM remains relatively constant for
...
keV.  \sh{The simulation end time is 5 sh.} For all HOLO simulations, we have used 8
...
acceleration is critical. \sh{Inclusion of Compton scattering in this algorithm, which would introduce
...
\bibliography{NSE16-36R1_LaTeX_References.bib}
\clearpage
\appendix

\ess

\section{\sh{Implementation of ECMC finite-element space, tallies, and residual sampling}}
...
problem.  \sh{The MC source $r^{(m)}(x,\mu)$ in Eq.~\eqref{eq:mc_err}
